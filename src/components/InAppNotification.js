            ...
            <View style={{ flexDirection:'row', width:'100%' }}>
                {
                    ...
                    && (<View style={{ width:'90%' }}>
                        <Text style={{ fontWeight: 'bold', color: 'black' }}>{notifications[notifications.length - 1]?.title}</Text>
                        <Text>{notifications[notifications.length - 1]?.body}</Text>
                    </View>)
                }
                <TouchableOpacity onPress={() => dispatch(setIncomingNotificationStatus(false))} style={styles.closeBtn}>
