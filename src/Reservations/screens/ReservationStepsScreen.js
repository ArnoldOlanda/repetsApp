...
import { ChoosePaymentMethodStep, ChoosePetStep, SelectDateTimeStep, ResumeReservation } from '../components';
    ...
    const onSubmitReservationDataForm = async () => {
        ...
        try {
            if (reservationData.metodo_pago === 'contado') {
                ...
                socket.emit("save-new-reservation", {
                    ...reservationData,
                    mascotas: reservationData.mascotas.map(m => m.uid),
                    fecha_solicitud: new Date()
                });
                console.log({
                    ...reservationData,
                    mascotas: mascotas.map(m => m.uid),
                    fecha_solicitud: new Date()
                });
                ...
                console.log(data);
                socket.emit("save-new-reservation", data);
